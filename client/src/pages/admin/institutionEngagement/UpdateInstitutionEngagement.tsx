...
export function UpdateInstitutionEngagementForm() {
  ...
  useEffect(() => {
    async function fetchEventData() {
      ...
      try {
        const response = await fetch(`https://50wqmqhq-5000.euw.devtunnels.ms/admin/institution-engagements/${id}`, {
          method: 'GET',
          headers: {
        ...
        if (data) {
          ...
          setExistingUrl(`https://50wqmqhq-5000.euw.devtunnels.ms/${volunteerPic}`);
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      if (data.volunteerPic && data.volunteerPic instanceof File) {
        formData.append('volunteerPic', data.volunteerPic); // Append file only if new file is selected
      }
