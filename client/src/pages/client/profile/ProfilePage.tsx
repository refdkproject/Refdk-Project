...
export default function ProfilePage() {
  ...
  const [profileData, setProfileData] = useState<
    VolunteerProfile | CharityProfile | any | null | undefined
  >(null)
...
const validateForm = () => {
  ...
  if (!trimmedEmail) {
    ...
    const isEmailTaken = allUsers.some(
      (user) => user.email === trimmedEmail && user.id !== profileData.id
    );
    if (isEmailTaken) {
  ...
  if (!trimmedPhoneNumber) {
    ...
    isValid = false;
  } else if (!/^\+966\d{9}$/.test(trimmedPhoneNumber)) {
    tempErrors.phoneNumber = "Phone number must start with '+966' and be 13 characters long";
    toast.error("Phone number must start with '+966' and be 13 characters long", { position: "top-right" });
    isValid = false;
  }
  ...
  if (profileData?.role === "volunteer" && !skillsInput) {
    tempErrors.skills = "At least one skill is required";
    isValid = false;
  }

  if (profileData?.role === "volunteer" && !areasInput) {
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  const updatedProfile = {
    ...
    areasOfInterest: areasInput.split(",").map((s) => s.trim()).filter(Boolean),
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-[#397260] mb-8">
          Profile Settings
        </h1>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="col-span-2">
              <h2 className="text-xl font-semibold text-[#397260] mb-4">
...
const VolunteerFields = ({
  ...
  setProfile: React.Dispatch<
    React.SetStateAction<
      VolunteerProfile | CharityProfile | null | any | undefined
    >
  >
  skillsInput: string
