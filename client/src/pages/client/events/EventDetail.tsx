...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchEvent = async () => {
      try {
        const response = await fetch(`https://50wqmqhq-5000.euw.devtunnels.ms/api/event/${eventId}`, {
          method: "GET",
          headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`https://50wqmqhq-5000.euw.devtunnels.ms/api/event/${eventId}/volunteer`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${user.token}`,
            ...
            {loading ? (
              ...
              <Card key={event._id} className="bg-white min-w-[50%]">
                <CardContent className="p-6">
                  {event.eventPic && (
                    <img
                      src={`https://50wqmqhq-5000.euw.devtunnels.ms/${event.eventPic.replace(/\\/g, "/")}`}
                      alt={event.name}
                  ...
                  <button
                    ...
                    className="mt-6 bg-[#2D5A4C] text-white py-2 px-4 rounded"
                  >
                    Join Event
                  </button>
                </CardContent>
